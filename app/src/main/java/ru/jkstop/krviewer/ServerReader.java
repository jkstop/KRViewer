...
/**
 * Created by ivsmirnov on 17.06.2016.
 */
        ...
        switch (mReadParam){
            case READ_ALL:
                try {
                    ...
                    *
                    ...
                    mJournalTags.addAll(JournalDB.getJournalItemsOpenTime(null));

                    //для каждой записи проверяем, не удалилась ли она с сервера. Коряво, но пока так.

                    ResultSet journalExistResult = mStatement.executeQuery("SELECT " + ServerConnect.COLUMN_JOURNAL_TIME_IN + " FROM " +ServerConnect.JOURNAL_TABLE);
                    ...
                    ResultSet getJournalTagsResult = connection.prepareStatement("SELECT " + ServerConnect.COLUMN_JOURNAL_TIME_IN
                                    + " FROM " + ServerConnect.JOURNAL_TABLE + " WHERE " + ServerConnect.COLUMN_JOURNAL_TIME_IN
              ...
              //      mHandler.sendEmptyMessage(HANDLER_UPDATE_JOURNAL);
                  ...
                  //      publishProgress(getJournalTagsResult.getRow());
                    ...
                    *
                    ...
                    ResultSet personsExistResult = mStatement.executeQuery("SELECT " + ServerConnect.COLUMN_PERSONS_TAG + " FROM " +ServerConnect.PERSONS_TABLE);
                    ...
                    ResultSet getPesonsTagsResult = connection.prepareStatement("SELECT " + ServerConnect.COLUMN_PERSONS_TAG + " FROM " + ServerConnect.PERSONS_TABLE
                                    + " WHERE " + ServerConnect.COLUMN_PERSONS_TAG + " NOT IN (" + getInClause(mPersonsTags) + ")"
                ...
                //    mHandler.sendEmptyMessage(HANDLER_UPDATE_PERSONS);
                    ...
                    while (getPesonsTagsResult.next()){
                        personItemResult = mStatement.executeQuery("SELECT * FROM " +ServerConnect.PERSONS_TABLE
                                + " WHERE " + ServerConnect.COLUMN_PERSONS_TAG
                        ...
                        if (personItemResult.getRow() != 0){
                            ...
                            .setInitials(UsersDB.createUserInitials(UsersDB.FULL_INITIALS,
                    ...
                    *
                    ...
                    for (Room room : roomList){
                        roomExistResult = mStatement.executeQuery("SELECT " + ServerConnect.COLUMN_ROOMS_ROOM + " FROM " +ServerConnect.ROOMS_TABLE
                                + " WHERE " + ServerConnect.COLUMN_ROOMS_ROOM + " = '" + room.getName() + "'");
                    ...
                    ResultSet getRoomsItemsResult = connection.prepareStatement("SELECT * FROM " + ServerConnect.ROOMS_TABLE
                                    + " WHERE " + ServerConnect.COLUMN_ROOMS_ROOM + " NOT IN (" + getInClause(roomNameList) + ")"
                            ,ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE).executeQuery();
                ...
                //    mHandler.sendEmptyMessage(HANDLER_UPDATE_ROOMS);
                    ...
                    mResult = mStatement.executeQuery("SELECT * FROM " + ServerConnect.ROOMS_TABLE);
            ...
            case READ_PERSON_ITEM:
                try {
                    ...
                    ResultSet resultSet = statement.executeQuery("SELECT * FROM " + ServerConnect.ALL_STAFF_TABLE
                            + " WHERE " + ServerConnect.COLUMN_ALL_STAFF_TAG + " = '" + mPersonItemTag + "'");
                    resultSet.first();
                    if (resultSet.getRow() !=0){
                        mServerUser = new User()
                                .setInitials(UsersDB.createUserInitials(UsersDB.FULL_INITIALS,
                                        resultSet.getString(ServerConnect.COLUMN_ALL_STAFF_LASTNAME),
                                ...
                                .setPhotoBinary(resultSet.getString(ServerConnect.COLUMN_ALL_STAFF_PHOTO))
  ...
  //  }
    ...
    protected void onPostExecute(Exception e) {
        ...
        if (mCallback!=null){
            if (e == null){
                if (mServerUser !=null){
                    mCallback.onSuccessServerRead(mReadParam, mServerUser);
                } else {
